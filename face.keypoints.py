...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        win_width = 640
        win_height = 360
        cv2.namedWindow('noGlass', 0)
        cv2.resizeWindow('noGlass', win_width, win_height)
        cv2.namedWindow('withGlass', 0)
        cv2.resizeWindow('withGlass', win_width, win_height)
        # 显示加了眼镜的图像
        cv2.imshow("withGlass", frame)
        # 显示添加了关键点的图像
        cv2.imshow("noGlass", frame2)
